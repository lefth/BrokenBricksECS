...
namespace ECS
{
    ...
    {
        static Type _componentType = typeof(TComponent);
        const int _startSize = 8;
        const int _resizeFactor = 2;
        readonly Dictionary<Entity, int> _componentsMap = new Dictionary<Entity, int>();


        Entity[] _entities = new Entity[_startSize];
        TComponent[] _components = new TComponent[_startSize];
        int _size;

        #region Events

        ComponentAddedToEntityEvent<TComponent> _typedComponentAddedToEntityEvent = new ComponentAddedToEntityEvent<TComponent>();
        ComponentRemovingFromEntityEvent<TComponent> _typedComponentRemovingFromEntityEvent = new ComponentRemovingFromEntityEvent<TComponent>();
        ComponentRemovedFromEntityEvent<TComponent> _typedComponentRemovedFromEntityEvent = new ComponentRemovedFromEntityEvent<TComponent>();
        ComponentChangedOfEntityEvent<TComponent> _typedComponentChangedOfEntityEvent = new ComponentChangedOfEntityEvent<TComponent>();
        ...
        public bool Add(Entity entity, TComponent component)
        {
            ...
            {
                var newEntityArray = new Entity[_entities.Length * _resizeFactor];
                var newComponentArray = new TComponent[_components.Length * _resizeFactor];
        ...
        public override bool Add(Entity entity, EntityManager entityManager)
        {
            ...
            var component = entityManager.GetComponent<TComponent>(entity);
            _entities[_size] = entity;
            _components[_size] = component;
            _componentsMap.Add(entity, _size);
            _size++;
        ...
        public override bool Remove(Entity entity)
        {
            ...
            if (_componentsMap.TryGetValue(entity, out index))
            {
                ...
                var shrinkSize = _components.Length / (2 * _resizeFactor);
                if (_size <= shrinkSize && shrinkSize > _startSize)
                {
                    var newEntityArray = new Entity[_entities.Length / _resizeFactor];
                    var newComponentArray = new TComponent[_components.Length / _resizeFactor];
                    Array.Copy(_entities, newEntityArray, _size);
        ...
        {
            int index;
            if (_componentsMap.TryGetValue(entity, out index)) component = _components[index];
            else component = null;
        }
