...
namespace ECS
{
    public class ComponentSystem : IComponentSystemSetup, IEntityAddedEventListener, IEntityRemovedEventListener, IEntityRemovingEventListener
    {
        ...
        public IEnumerable<ComponentGroup> Groups { get { return _groups.Keys; } }

        readonly Dictionary<ComponentGroup, string> _groups = new Dictionary<ComponentGroup, string>();
        ...
        {
            string groupName;
            if (_groups.TryGetValue(sender as ComponentGroup, out groupName)) OnEntityAdded(groupName, entity);
        }

        void IEntityRemovedEventListener.OnEntityRemoved(object sender, Entity entity)
        {
            string groupName;
            if (_groups.TryGetValue(sender as ComponentGroup, out groupName)) OnEntityRemoved(groupName, entity);
        }

        void IEntityRemovingEventListener.OnEntityRemoving(object sender, Entity entity)
        {
            string groupName;
            if (_groups.TryGetValue(sender as ComponentGroup, out groupName)) OnEntityRemoving(groupName, entity);
        }
    }
